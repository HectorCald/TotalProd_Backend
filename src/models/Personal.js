...
class Personal {
  ...
  // Método para paginación con búsqueda
  static async getAllPaginated(empresaId, { page, limit, offset, search }) {
    try {
      if (!empresaId) {
        throw new Error('ID de la empresa es requerido');
      }
      ...
      if (search) {
        query = query.or(`first_name.ilike.%${search}%,last_name.ilike.%${search}%,codigo.ilike.%${search}%`);
      }
      ...
      return {
        personal: processedData,
        total: count || 0
      };
    } catch (error) {
      console.error('Error al obtener el personal paginado:', error);
      throw new Error('No se pudo obtener el personal');
    }
  }
